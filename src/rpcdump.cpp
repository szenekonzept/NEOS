...
Value importprivkey(const Array& params, bool fHelp)
{
    if (fHelp || params.size() < 1 || params.size() > 3)
        throw runtime_error(
            "importprivkey <neoscoinprivkey> [label] [rescan=true]\n"
...
Value dumpprivkey(const Array& params, bool fHelp)
{
    if (fHelp || params.size() != 1)
        throw runtime_error(
            "dumpprivkey <neoscoinaddress>\n"
            "Reveals the private key corresponding to <neoscoinaddress>.");
    ...
    if (!address.SetString(strAddress))
        throw JSONRPCError(RPC_INVALID_ADDRESS_OR_KEY, "Invalid NeosCoin address");
    CKeyID keyID;
...
Value dumpwallet(const Array& params, bool fHelp)
{
    ...
    // produce output
    file << strprintf("# Wallet dump created by NeosCoin %s (%s)\n", CLIENT_BUILD.c_str(), CLIENT_DATE.c_str());
